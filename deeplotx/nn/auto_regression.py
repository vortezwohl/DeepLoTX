...
class AutoRegression(RecursiveSequential):
    def __init__(self, feature_dim: int, hidden_dim: int | None = None, bias: bool = True,
                 recursive_layers: int = 1, ffn_layers: int = 1, ffn_expansion_factor: int | float = 2,
                 dropout_rate: float = 0.05, model_name: str | None = None, device: str | None = None,
                 dtype: torch.dtype | None = None):
        super().__init__(input_dim=feature_dim, output_dim=feature_dim, bias=bias,
                         recursive_layers=recursive_layers, recursive_hidden_dim=hidden_dim,
